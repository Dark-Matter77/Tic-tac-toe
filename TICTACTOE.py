...
window = tkinter.Tk()   #create the game windows
window.title("Tic Tac Toe")
window.resizable(False,False)     #user can't change the size of windows
...
for row in range(3):
    for column in range(3):
        board[row][column] = tkinter.Button(frame, text="", font=("Consolas", 50, "bold"),
                                           ...
                                           command = lambda row=row, column=column: set_tile(row, column)) #this gets each button its own row and column when clicked
...
#format "(w) x (h) + (x) + (y)"
window.geometry(f"{window_width}x{window_height}+{window_x}+{window_y}") #create a window of size 300x200 and position it at(400,150) on the screen

window.mainloop()   #windows activate until the user click on "X"
